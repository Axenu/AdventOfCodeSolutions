input_data = '''vpryah (310) -> iedlpkf, epeain
aqfemi (38)
...
def strip_weight(weight):
    return int(weight.rstrip(')').strip('('))
...
for row in rows:
    columns = row.split(' ')
    length = len(columns)
    n = Node(columns[0], strip_weight(columns[1]))
    while length > 3:
        not_bottom.append(columns[length-1].rstrip(','))
        n.childNames.append(columns[length-1].rstrip(','))
        length -= 1
    nodes.append(n)
...
for row in rows:
    columns = row.split(' ')
    if columns[0] not in not_bottom:
        print columns[0]
        root = columns[0]

for node in nodes:
    for child_name in node.childNames:
        for n in nodes:
            if child_name == n.name:
