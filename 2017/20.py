input_data = '''p=<-833,-499,-1391>, v=<84,17,61>, a=<-4,1,1>
...
# input_data = '''p=<3,0,0>, v=<2,0,0>, a=<-1,0,0>
...
class Particle:
    def __init__(self, row):
        vals = row.split(', ')
        p = vals[0][3:-1].split(',')
        self.p = Point(p[0], p[1], p[2])
        v = vals[1][3:-1].split(',')
        self.v = Point(v[0], v[1], v[2])
        a = vals[2][3:-1].split(',')
        self.a = Point(a[0], a[1], a[2])
...
for j in xrange(0, 10000):
    ...
    for x in xrange(0, len(particles)):
        for y in xrange(x+1, len(particles)):
            if particles[x].p == particles[y].p:
                markForDelete.append(particles[x])
                markForDelete.append(particles[y])
    for p in markForDelete:
        if p in particles:
            particles.remove(p)
