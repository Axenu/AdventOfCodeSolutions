input_data = '''value 23 goes to bot 208
...
def addToBot(bots, bid, value, actions):
    v = bots[bid]
    if v[0] == -1:
        bots[bid][0] = value
    elif v[1] == -1:
        ...
        doBotAction(bots, bid, actions)
        addToBot(bots, bid, value, actions)
    v = bots[bid]
    if v[0] == 17 and v[1] == 61:
        print bid
    if v[1] == 17 and v[0] == 61:
        print bid

def doBotAction(bots, bid, actions):
    bot = bots[bid]
    action = actions[bid]
    if bot[0] < bot[1]:
        if action[2] == 1:
            addToBot(bots, action[0], bot[0], actions)
        else:
            outputs[action[0]].append(bot[0])
        bot[0] = -1
        if action[3] == 1:
            addToBot(bots, action[1], bot[1], actions)
        else:
            outputs[action[1]].append(bot[1])
        bot[1] = -1
    else:
        if action[3] == 1:
            addToBot(bots, action[1], bot[0], actions)
        else:
            outputs[action[1]].append(bot[0])
        bot[0] = -1
        if action[2] == 1:
            addToBot(bots, action[0], bot[1], actions)
        else:
            print outputs[action[1]]
            outputs[action[0]].append(bot[1])
        bot[1] = -1
...
for row in rows:
    ...
    if words[0] == 'bot':
        ...
        low = int(words[6])
        high = int(words[11])
        actions[bot] = [low, high, lowT, highT]
    else:
        value = int(words[1])
        bot = int(words[5])
        addToBot(bots, bot, value, actions)

for i in xrange(0, 100):
    bid = 0
    # print bots
    for bot in bots:
        if bot[0] != -1 and bot[1] != -1:
            doBotAction(bots, bid, actions)
        bid += 1
