input_data = '''hqcfqwydw-fbqijys-whqii-huiuqhsx-660[qhiwf]
...
# input_data = '''bbb-aaaaa-z-y-x-123[abxyz]
...
def decrypt(input, sid):
    answer = ''
    for word in input:
        for char in list(word):
            v = ord(char)
            for i in xrange(0, sid):
                v += 1
                if v > 122:
                    v = 97
            answer += chr(v)
        answer += ' '
    print str(sid) + ': ' + answer
